...
~~~
使用最普遍的引用。不会被 GC 回收。当内存不足，抛出 OutOfMmoryError 错误使程序异常终止。
~~~
...
~~~
SoftReference<> 有用但非必须的对象，适用于缓存。当内存不足，就会进行一次 GC ，但不会回收软引用对象，只是纳入回收范围，当 GC 后任内存不足，则进行第二次 GC 回收软引用对象。
~~~
...
~~~
WeakReference<> 非必须对象，强度比软引用更弱一些，被弱引用关联的对象，每次 GC 都会被回收。
...
public static void main(String[] args) {
    ...
    // 当 这个 key 不在需要使用了之后，通过 key = null 是不能回收这个对象的，因为 container 
    key = null;
}
~~~
